...
<body>
  ...
  <div class="footer">Generated by KaiJar &middot; ${esc(generatedOn)}</div>
</body>
</html>`;
}
...
export async function exportMonthlyReportPdf(yearMonth: string): Promise<void> {
  ...
  if (Platform.OS === 'web') {
    ...
    await downloadOrShareFile(html, `kaijar-report-${yearMonth}.html`, 'text/html', 'Monthly Report');
    return;
  }
  ...
  try {
    ...
    // something a person can find later ("KaiJar - August 2026.pdf").
    const shareUri = await renameForSharing(uri, `KaiJar - ${formatMonthLabel(yearMonth)}.pdf`);

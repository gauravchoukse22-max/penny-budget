...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    changes: [
      ...
      'Imported charges land in the right month. Statement rows usually print as just "06/14" with no year, and the year was being taken from the largest one printed anywhere on the page — which on a statement offering a 0% rate "until 2027" meant a whole June statement was filed a year into the future, where nothing would ever show it. The year now comes from the statement\'s own closing date, so a December charge on a January statement stays in December too.',
      'After an import, the app moves to the month the transactions landed in and says which month that is, instead of leaving you on an empty screen wondering whether it worked.',
